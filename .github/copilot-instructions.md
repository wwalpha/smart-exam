...
- ロジックが複雑な箇所や、ビジネスルールに関わる重要な処理（例: 排他制御、特殊なフィルタリング）には、必ず行レベルの日本語コメントを追加してください。

### 1.3 関数定義のネスト禁止（再発防止）

- `backend/src/services/exam/**` では「関数の中で新たな関数を定義」する実装を禁止します。
- 対象には `function` / `const fn = () => {}` / `return async (...) => {}` を含みます。
- ヘルパー関数はファイルトップレベルに切り出し、factory は `bind` などで依存注入した関数を返してください。
