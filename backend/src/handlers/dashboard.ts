...
const toUtcDateKey = (iso: string): string => {
  // startedAt/submittedAt are ISO strings; dashboard wants YYYY-MM-DD
...
export const getDashboard: AsyncHandler<{}, DashboardData, {}, ParsedQs> = async (_req, res) => {
  ...
  const todayTestCount = attempts.filter((a) => (a.startedAt ? toUtcDateKey(a.startedAt) === todayKey : false)).length;
