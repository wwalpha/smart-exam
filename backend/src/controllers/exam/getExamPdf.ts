...
import { GetExamPdfParamsSchema, GetExamPdfQuerySchema } from './exam.schema';
...
export const getExamPdfController = (services: Services) => {
  ...
  > = async (req, res) => {
    ...
    const { examId } = (req.validated?.params ?? req.params) as ValidatedParams<typeof GetExamPdfParamsSchema>;
    // クエリ文字列のオプション値を取得する
    ...
    if (direct) {
      ...
      const filename = `review-test-${examId}.pdf`;
