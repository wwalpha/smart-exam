...
export const createCreateReviewTest = (deps: {
  ...
  deleteReviewTest: ReviewTestsService['deleteReviewTest'];
}): ReviewTestsService['createReviewTest'] => {
  ...
  return async (req): Promise<ReviewTest> => {
    ...
    if (req.mode === 'KANJI' && pdfS3Key) {
      try {
        ...
        const detail = await deps.getReviewTest(testId);
        if (!detail) {
        ...
        await deps.deleteReviewTest(testId);
        throw e;
      }
    }
