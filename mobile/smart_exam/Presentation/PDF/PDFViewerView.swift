...
struct PDFViewerView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            } else if let descriptor = viewModel.state.descriptor {
                viewerArea(descriptor: descriptor)
            } else {
    ...
    private func viewerArea(descriptor: PDFDocumentDescriptor) -> some View {
        ScrollView([.vertical, .horizontal]) {
            PDFKitView(url: descriptor.url, zoom: zoom)
