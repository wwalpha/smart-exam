...
本書は [要件定義\_v1.2.md](../要件定義_v1.2.md) をベースに、Smart Exam のフロントエンド画面一覧、ルーティング、画面詳細を定義します。

- Backend API 定義: [backend_api.md](backend_api.md)
- Swagger 定義: [swagger.yml](swagger.yml)
...
| 問題番号自動抽出（四谷大塚） | 四谷大塚解答から問題番号を自動抽出  | Phase 2（AI/OCR処理）          |
...
| `/exam/questions`                          | 復習テスト一覧（問題）     | 問題テストの一覧         |
| `/exam/questions/new`                      | 復習テスト生成（問題）     | 問題テスト生成フォーム   |
| `/exam/questions/:testid`                  | 復習テスト詳細（問題）     | テスト内容表示           |
| `/exam/questions/:testid/grading`          | 復習テスト結果入力（問題） | 結果入力フォーム         |
| `/exam/questions/:testid/pdf`              | 問題テストPDFプレビュー    | PDF生成・印刷            |
| `/exam/kanji`                              | 復習テスト一覧（漢字）     | 漢字テストの一覧         |
| `/exam/kanji/new`                          | 復習テスト生成（漢字）     | 漢字テスト生成フォーム   |
| `/exam/kanji/:testid`                      | 復習テスト詳細（漢字）     | テスト内容表示           |
| `/exam/kanji/:testid/grading`              | 復習テスト結果入力（漢字） | 結果入力フォーム         |
| `/exam/kanji/:testid/pdf`                  | 漢字テストPDFプレビュー    | PDF生成・印刷            |
...
- 任意項目：学年、回・テスト名、教材種別（SAPIX/四谷大塚）、単元、教室/コース/クラス、メモ、追加キーワード
...
### 4.10 復習テスト一覧 (問題) (`/exam/questions`)
...
- **GET** `/v1/exam/questions?subject={科目}&status={ステータス}&...` : テスト一覧取得
  - Query: 科目、作成日範囲、ステータス
  - Response: テスト一覧
- **DELETE** `/v1/exam/:testid` : テスト削除
  - Response: 成功/失敗

---

### 4.11 復習テスト一覧 (漢字) (`/exam/kanji`)
...
- **GET** `/v1/exam/kanji?subject={科目}&status={ステータス}&...` : テスト一覧取得
  - Query: 科目、作成日範囲、ステータス
  - Response: テスト一覧
- **DELETE** `/v1/exam/:testid` : テスト削除
  - Response: 成功/失敗
...
### 4.12 復習テスト生成 (問題) (`/exam/questions/new`)
...
- **POST** `/v1/exam/questions` : 問題テスト生成
  - Request: 科目、出題数、対象期間
  - Response: testId、指定件数、生成件数、不足数

---

### 4.13 復習テスト生成 (漢字) (`/exam/kanji/new`)
...
- **POST** `/v1/exam/kanji` : 漢字テスト生成
  - Request: 科目、出題数、対象期間
  - Response: testId、指定件数、生成件数、不足数

---

### 4.14 復習テスト詳細 (問題) (`/exam/questions/:testid`)
...
- **GET** `/v1/exam/:testid` : テスト詳細取得
  - Response: テスト情報、出題項目一覧、元教材情報
- **PUT** `/v1/exam/:testid/status` : ステータス変更
  - Request: status（COMPLETED/PAUSED/CANCELED）
  - Response: 成功/失敗

---

### 4.15 復習テスト詳細 (漢字) (`/exam/kanji/:testid`)
...
### 4.16 復習テスト結果入力 (問題) (`/exam/questions/:testid/grading`)
...
- **POST** `/v1/exam/:testid/attempts` : 結果一括登録
  - Request: 各問題の結果（正解/不正解）、実施日
  - Response: 成功/失敗

---

### 4.17 復習テスト結果入力 (漢字) (`/exam/kanji/:testid/grading`)
...
- **POST** `/v1/exam/:testid/kanjiattempts` : 結果一括登録
  - Request: 各漢字の結果（正解/不正解）、実施日
  - Response: 成功/失敗

---

### 4.18 漢字テストPDFプレビュー (`/exam/kanji/:testid/pdf`)
...
- **GET** `/v1/exam/:testid/pdf?type={問題|解答}` : PDF生成
  - Query: type（問題/解答）
  - Response: PDF URL

---

### 4.19 問題テストPDFプレビュー (`/exam/questions/:testid/pdf`)
...
3. 復習テスト生成
   ↓
  /exam/questions/new で科目・出題数を指定してテスト生成
   ↓
4. テスト詳細確認
   ↓
  /exam/questions/:testid でテスト内容を確認
   ↓
5. PDF生成・印刷（任意）
   ↓
  /exam/questions/:testid/pdf でPDF生成・印刷
   ↓
6. テスト実施（オフライン）
   ↓
7. 結果入力
   ↓
  /exam/questions/:testid/grading で結果を一括入力
   ↓
8. ダッシュボード確認
   ↓
...
2. 復習テスト生成
   ↓
  /exam/kanji/new で科目・出題数を指定してテスト生成
   ↓
3. テスト詳細確認
   ↓
  /exam/kanji/:testid でテスト内容を確認
   ↓
4. PDF生成・印刷
   ↓
  /exam/kanji/:testid/pdf で問題用PDF・解答用PDFを生成・印刷
   ↓
5. テスト実施（オフライン）
   ↓
6. 結果入力
   ↓
  /exam/kanji/:testid/grading で結果を一括入力
   ↓
7. ダッシュボード確認
   ↓
   / で不正解率ランキング・次回テスト予定を確認
...
本ドキュメントは [要件定義\_v1.2.md](../要件定義_v1.2.md) のユースケースを網羅しています。
...
本ドキュメントは、要件定義\_v1.2.md をベースに、Smart Exam のフロントエンド画面を体系的に整理しました。
